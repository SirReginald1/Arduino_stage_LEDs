...
class Microphone{
    public:
        ...
        static int16_t sBuffer[bufferLen];

        static float readingsWindow[hopLength];
...
int16_t Microphone::sBuffer[bufferLen] = {};

float Microphone::readingsWindow[hopLength] = {};
...
size_t Microphone::readMic(void* buffer){
    ...
    size_t bytesIn = 0;
    i2s_read(I2S_PORT, buffer, bufferLen * sizeof(long), &bytesIn, portMAX_DELAY);
    return bytesIn;
}
